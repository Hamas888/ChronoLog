...
int main(void)
{
  ...
  logger.debug("STM32L010 Logging Example Starting...");
  ...
  while (1)
  {
    ...
    logger.info("Main loop iteration: %d", ++counter);
    
    sensorLogger.debug("Reading sensor... Value: %d", counter * 10);
    
    if (counter % 5 == 0) {
      logger.warn("Periodic system check");
    }
